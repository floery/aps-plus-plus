...
class Gun extends EventEmitter {
  constructor(body, info) {
    ...
    if (info.PROPERTIES != null) {
      ...
      if (info.PROPERTIES.STROKE_WIDTH != null)
        this.strokeWidth = info.PROPERTIES.STROKE_WIDTH;
      if (info.PROPERTIES.BORDERLESS != null)
        this.borderless = info.PROPERTIES.BORDERLESS;
      if (info.PROPERTIES.DRAW_FILL != null)
        this.drawFill = info.PROPERTIES.DRAW_FILL;
      if (info.PROPERTIES.DRAW_ABOVE)
        this.drawAbove = info.PROPERTIES.DRAW_ABOVE;
  ...
  live() {
    if (!this.canShoot || this.codeControlOnly || this.body.master.invuln)
      return;
    ...
    if ((shootPermission || !this.waitToCycle) && this.cycleTimer < 1) {
      this.cycleTimer +=
        1 /
        (this.shootSettings.reload * this.reloadRateFactor * Config.runSpeed);
    }
  ...
  checkShootPermission() {
    let shootPermission = this.maxChildren
      ? this.maxChildren > this.children.length * this.childrenLimitFactor
  ...
  fire() {
    ...
    this.lastShot.power =
      3 * Math.log(Math.sqrt(this.bulletSkills.spd) + this.trueRecoil + 1) + 1;
  ...
  findBulletSpawnPosition() {
    ...
    let offsetAngle = this.offsetDirection + this.angle + this.body.facing,
      gunlength =
        this.length +
        (Config.bulletSpawnOffset * this.width * this.shootSettings.size) / 2,
  ...
  createBullet(spawnX, spawnY) {
    ...
    if (this.shootSettings.spray) {
      do {
        spread = ran.gauss(
          0,
          this.shootSettings.spray * this.shootSettings.shudder
        );
    ...
    let velocityMagnitude =
        this.negativeRecoil *
        this.shootSettings.speed *
        this.bulletSkills.spd *
        (shudder + 1) *
        Config.runSpeed,
      velocityDirection = this.angle + this.body.facing + spread,
      velocity = new Vector(
        velocityMagnitude * Math.cos(velocityDirection),
        velocityMagnitude * Math.sin(velocityDirection)
      );
    ...
    if (this.body.velocity.length) {
      let extraBoost =
        Math.max(
          0,
          velocity.x * this.body.velocity.x + velocity.y * this.body.velocity.y
        ) /
      ...
      if (extraBoost) {
        ...
        velocity.x +=
          (this.body.velocity.length * extraBoost * velocity.x) / len;
        velocity.y +=
          (this.body.velocity.length * extraBoost * velocity.y) / len;
      }
    }
  ...
  defineBullet(bullet) {
    ...
    this.bulletType.SIZE =
      (this.body.size * this.width * this.shootSettings.size) / 2;
    bullet.define(this.bulletType);
  ...
  calculateBulletStats() {
    ...
    let out = {
      ...
      DENSITY:
        (shoot.density * this.bulletSkills.pen * this.bulletSkills.pen) /
        sizeFactor,
    ...
    switch (this.statCalculator) {
      case "thruster":
        this.trueRecoil =
          shoot.recoil *
          Math.sqrt(this.bulletSkills.rld * this.bulletSkills.spd);
        break;
      case "sustained":
      ...
      case "swarm":
        out.PENETRATION = Math.max(
          1,
          shoot.pen * (0.5 * (this.bulletSkills.pen - 1) + 1)
        );
      ...
      case "drone":
        ...
        out.HEALTH =
          (shoot.health * this.bulletSkills.str + sizeFactor) /
          Math.pow(this.bulletSkills.pen, 0.8);
        out.DAMAGE =
          shoot.damage *
          this.bulletSkills.dam *
          Math.sqrt(sizeFactor) *
          shoot.pen *
          this.bulletSkills.pen;
    ...
    for (let property in out) {
      if (
        this.bulletBodyStats[property] == null ||
        !out.hasOwnProperty(property)
      )
        continue;
  ...
  getTracking() {
    return {
      speed:
        Config.runSpeed *
        this.bulletSkills.spd *
        this.shootSettings.maxSpeed *
        this.bulletBodyStats.SPEED,
      range:
        Math.sqrt(this.bulletSkills.spd) *
        this.shootSettings.range *
  ...
  define(defs, emitEvent = true) {
    ...
    if (set.CONTROLLERS != null) {
      ...
      for (let i = 0; i < set.CONTROLLERS.length; i++) {
        ...
        if (defs === "mlg") { console.log(ioTypes[io[0]], io[0]); }
    ...
    if (set.NECRO != null) {
      ...
      for (let shape of this.settings.necroTypes) {
        ...
        this.settings.necroDefineGuns[shape] = this.guns.filter(
          (gun) =>
            gun.bulletType.NECRO === shape ||
            (gun.bulletType.NECRO === true &&
              gun.bulletType.SHAPE === this.shape) ||
            gun.bulletType.NECRO.includes(shape)
        )[0];
      }

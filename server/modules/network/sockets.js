  ...
  if (index != -1) {
    ...
    util.log(
      "[INFO] " +
        (player.body
          ? `User ${
              player.body.name == "" ? "A unnamed player" : player.body.name
            }`
...
function incoming(message, socket) {
  ...
  switch (m.shift()) {
    case "k":
      ...
      if (m.length === 1) {
        ...
        if (socket.permissions) {
          util.log(
            `[INFO] A socket ( ${socket.ip} ) was verified with the token: ${key}`
          );
        } else {
          util.log(
            `[WARNING] A socket ( ${socket.ip} ) failed to verify with the token: ${key}`
          );
        }
        socket.key = key;
      }
      ...
      util.log(
        `[INFO] A socket ( ${socket.ip} ) has been welcomed to the server room. Waiting for spawn request.`
      );
    ...
    case "s":
      // spawn request
      util.log(
        `[INFO] A socket ( ${socket.ip} ) is asking for spawn request, checking all securities...`
      );
      ...
      // Log it
      util.log(
        `[INFO] ${name == "" ? "An unnamed player" : m[0]} ${
          needsRoom ? "joined" : "rejoined"
        } the game on team ${socket.player.body.team}! Players: ${
          players.length
        }`
      );
      break;
    ...
    case "M":
      ...
      if (message[0] === "/") {
        ...
        const target = findTargetClient(clients, parameters[0]);

        if (!handleCommand(command, parameters, socket, player, target)) {
          player.body.sendMessage("Unknown command or insufficient permissions.");
        }
      }

      function findTargetClient(clients, targetId) {
        return clients.find(c => c.player.body && c.player.body.id === parseInt(targetId));
      }

      function handleCommand(command, parameters, socket, player, target) {
        const commands = {
          color: handleColorCommand,
          invulnerable: handleInvulnerableCommand,
          stronger: handleStrongerCommand,
          size: handleSizeCommand,
          fov: handleFovCommand,
          report: handleReportCommand,
          id: handleIdCommand,
          define: handleDefineCommand,
          developer: handleDeveloperCommand,
          alert: handleAlertCommand,
          ban: handleBanCommand,
          list: handleListCommand,
          broadcast: handleBroadcastCommand,
          req: handleReqCommand,
          room_size: handleRoomSizeCommand,
        ...
        if (handler) {
          return handler(socket, player, target, parameters);
        }

        return false;
      }

      function handleColorCommand(socket, player, target, parameters) {
        if (!socket.operator) return false;

        if (parameters[0]) {
          player.body.color.base = parseInt(parameters[0], 10);
        } else {
          player.body.sendMessage(`Your color is now ${player.body.color.base}.`);
        }
        return true;
      }

      function handleInvulnerableCommand(socket, player) {
        if (!socket.operator) return false;

        player.body.opInvuln = !player.body.opInvuln;
        player.body.sendMessage(player.body.opInvuln ? "Activated invulnerability." : "Deactivated invulnerability.");
        return true;
      }

      function handleStrongerCommand(socket, player, target, parameters) {
        if (!socket.operator) return false;

        const v = parseInt(parameters[0], 10);
        player.body.skill.setCaps([v, v, v, v, v, v, v, v, v, v]);
        player.body.skill.set([v, v, v, v, v, v, v, v, v, v]);
        return true;
      }

      function handleSizeCommand(socket, player, target, parameters) {
        if (!socket.operator) return false;
      ...
      function handleFovCommand(socket, player, target, parameters) {
        if (!socket.operator) return false;

        if (parameters.length === 2 && target) {
      ...
      function handleReportCommand(socket, player, target, parameters) {
        if (!target) return false;

        const reportMessage = `"${player.body.name}" reported "${target.name}" for reason: "${parameters[1]}"`;
        console.error(reportMessage);

        clients.forEach(client => {
          if (client.dev) {
            client.player.body.sendMessage(reportMessage);
          }
        });
        return true;
      }

      function handleIdCommand(socket, player) {
        player.body.sendMessage(`Your ID is ${player.body.id}.`);
        return true;
      }

      function handleDefineCommand(socket, player, target, parameters) {
        if (!socket.verified) return false;

        try {
          if (parameters.length >= 2 && target && target.player.body) {
            target.player.body.define(parameters[1]);
          } else if (parameters.length === 1) {
            player.body.define(parameters[0]);
          } else {
            player.body.sendMessage("This command requires at least 1 parameter.");
          }
        } catch (e) {
      ...
      function handleDeveloperCommand(socket, player) {
        if (!socket.verified) return false;
      ...
      function handleAlertCommand(socket, player, target, parameters) {
        if (!socket.verified || !target) return false;

        target.talk("P", parameters[1]);
        return true;
      }

      function handleBanCommand(socket, player, target) {
        if (!socket.verified || !target) return false;
      ...
      function handleListCommand(socket, player) {
        clients.forEach(c => {
          player.body.sendMessage(`"${c.player.body.name}": ${c.player.body.id}`);
        });
        return true;
      }

      function handleBroadcastCommand(socket, player, target, parameters) {
        if (!socket.verified) return false;
      ...
      function handleReqCommand(socket, player) {
        if (!socket.verified) return false;
      ...
      function handleRoomSizeCommand(socket, player) {
        if (!socket.operator) return false;

        player.body.sendMessage(`The room size is ${room.width} x ${room.height}.`);
        return true;
      }

      function handleCloseCommand(socket) {
        if (!socket.verified) return false;
...
const sockets = {
  ...
  connect: (socket, req) => {
    ...
    // Log it
    clients.push(socket);
    util.log("[INFO] New socket opened with id " + socket.id);
  },
};

...
(async function (util, global, settings, Canvas, color, gameDraw, socketStuff) {
  ...
  function startGame() {
    ...
    if (document.getElementById("optMobile").value === "desktop")
      global.mobile = false;
    // Name and keys
  ...
  function drawMobileButtons(spacing, alcoveSize) {
    ...
    let yOffset = global.mobile
      ? global.canUpgrade
        ? (((alcoveSize / 3.5) /*+ spacing * 2*/ *
            mobileUpgradeGlide.get() *
            upgradeColumns) /
            3.5) *
            (upgradeColumns + 3.55) +
          67
        : 0 + global.canSkill || global.showSkill
